...
const emit = async (eventType, context = {}) => {
  ...
  const events = {
    ...
    // NOTE: pcMeeting.controller passes each UDA user_id as `chairmanId` when iterating.
    // Both IN_APP and EMAIL channels are fired so UDA is reliably informed of new meetings.
    UDA_MEETING_SCHEDULED: [
      { recipient_id: chairmanId, channel: 'IN_APP', title: 'PC Meeting Scheduled',            body: `A new Planning Committee meeting has been scheduled. Please log in to the portal to view the agenda and meeting date.` },
      { recipient_id: chairmanId, channel: 'EMAIL',  title: 'PC Meeting Scheduled — Kelaniya PS', body: `A Planning Committee meeting has been scheduled at the Kelaniya Pradeshiya Sabha. Please log in to the planning portal to view the agenda and confirm your attendance.`, metadata: { email: applicantEmail } },
    ],
